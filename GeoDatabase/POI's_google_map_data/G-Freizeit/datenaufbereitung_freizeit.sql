...
select 
	*
from 
	google_maps_dev.google_map_items_freizeit_all t0
where 
	category_ids = 'null'
	or
	exists (
	    ...
	    where
	    	...
	    	and 
	    	t2.hauptkategorie_neu = 'Freizeit'
	)  
;
select * from tmp_freizeit_category_v1;
...
set  
    category_ids_de = 
    	case 
            when additional_categories is null or additional_categories::text = 'null' then to_jsonb(array[category])
            when jsonb_typeof(additional_categories::jsonb) = 'array' then jsonb_insert(additional_categories::jsonb, '{0}', to_jsonb(category))
            else to_jsonb(array[category] || additional_categories::text)
        end 
;
select * from tmp_freizeit_category_v1;
...
;
select * from tmp_freizeit_category;
...
group by 
	...
	,category_ids
;
select * from tmp_google_map_freizeit;
...
select
	*
from 
	google_maps_dev.google_map_freizeit
;
*/
...
--=================================
create index if not exists idx_tmp_googl_missing_hostel_poly_lv95 on tmp_google_map_freizeit using GIST(geo_point_lv95);
...
where 
    rn = 1
;
-- select * from tmp_google_map_freizeit_v1;
...
update google_maps_dev.google_map_freizeit
set 
	korr_plz4 = '8004'
where 
...
drop table if exists 
	google_maps_dev_abgleich.google_abgleich_freizeit;
...
--POIS: Google 
/*
select * from google_maps_dev_abgleich.google_abgleich_freizeit order by random() limit 100;

select count(*) from google_maps_dev_abgleich.google_abgleich_freizeit;
*/
	
--POIS: AFO
drop table if exists
	google_maps_dev_abgleich.afo_poi_typ_freizeit;
...
/*
select * from google_maps_dev_abgleich.afo_poi_typ_freizeit order by random() limit 100;

select count(*) from google_maps_dev_abgleich.afo_poi_typ_freizeit;
*/
...
(
	'google_maps_dev_abgleich.afo_poi_typ_freizeit'						-- afo_poi_input_table
	,'google_maps_dev_abgleich.google_abgleich_freizeit'					-- google_poi_input_table
	,'google_maps_dev_abgleich.poi_abgleich_google_freizeit_tot'			-- abgleich_output_table
)
;

/*
select * from google_maps_dev_abgleich.poi_abgleich_google_freizeit_tot order by dubletten_nr, quelle;
select count(*) from google_maps_dev_abgleich.poi_abgleich_google_freizeit_tot where quelle='AFO' and dubletten_nr is not null;
select count(*) from google_maps_dev_abgleich.poi_abgleich_google_freizeit_tot where quelle='GOOGLE' and dubletten_nr is null;
*/
	...
	aws_s3.query_export_to_s3(
		'	
			select 
				* 
			from 
				google_maps_dev_abgleich.poi_abgleich_google_freizeit_tot
		', 
   		aws_commons.create_s3_uri(
   			'afo-db-prod-data-exchange', 
   			'google_maps_dev_abgleich/poi_abgleich_google_freizeit_tot.csv', 
   			'eu-central-1'
   		),
