...
-- pois number: 18650
create temp table 
	tmp_religioese_einrichtung_category_v1
as
select 
	*
from 
	google_maps_dev.google_map_items_religiöse_einrichtung t0
where 
	category_ids = 'null'
	or
	exists (
	    ...
	    where
	    	...
	    	and 
	    	t2.hauptkategorie_neu = 'Religiöse Einrichtungen'
	)  
;
select * from tmp_religioese_einrichtung_category_v1;
...
drop table if exists 
	tmp_religioese_einrichtung_category;
...
--===============================================
drop table if exists 
	tmp_google_map_religioese_einrichtung;
...
select
	...
	,t0.latitude
from 
	tmp_religioese_einrichtung_category t0
left join
...
group by 
	...
	,category_ids
;
select * from tmp_google_map_religioese_einrichtung;
...
select
	*
from 
	google_maps_dev.google_map_religioese_einrichtung
;
*/
...
update
	tmp_google_map_religioese_einrichtung t0
set
	plz6 = t1.plz6
from
...
--=================================
create index if not exists idx_tmp_googl_missing_hostel_poly_lv95 on tmp_google_map_religioese_einrichtung using GIST(geo_point_lv95);
...
drop table if exists 
	tmp_google_map_religioese_einrichtung_V1;
...
where 
    rn = 1
;
-- select * from tmp_google_map_religioese_einrichtung_v1;
...
update 
	tmp_google_map_religioese_einrichtung_V1 t0
set 
...
--=====================================
drop table if exists 
	google_maps_dev.google_map_religioese_einrichtung_v1;
...
update google_maps_dev.google_map_religioese_einrichtung
set 
	korr_plz4 = '8004'
where 
...
drop table if exists 
	google_maps_dev_abgleich.google_abgleich_religioese_einrichtung;
...
--POIS: Google 
/*
select * from google_maps_dev_abgleich.google_abgleich_religioese_einrichtung order by random() limit 100;

select count(*) from google_maps_dev_abgleich.google_abgleich_religioese_einrichtung;
*/
	
--POIS: AFO
drop table if exists
	google_maps_dev_abgleich.afo_poi_typ_religioese_einrichtung;
...
/*
select * from google_maps_dev_abgleich.afo_poi_typ_religioese_einrichtung order by random() limit 100;

select count(*) from google_maps_dev_abgleich.afo_poi_typ_religioese_einrichtung;
*/
...
(
	'google_maps_dev_abgleich.afo_poi_typ_religioese_einrichtung'						-- afo_poi_input_table
	,'google_maps_dev_abgleich.google_abgleich_religioese_einrichtung'					-- google_poi_input_table
	,'google_maps_dev_abgleich.poi_abgleich_google_religioese_einrichtung_tot'			-- abgleich_output_table
)
;

/*
select * from google_maps_dev_abgleich.poi_abgleich_google_religioese_einrichtung_tot order by dubletten_nr, quelle;
select count(*) from google_maps_dev_abgleich.poi_abgleich_google_religioese_einrichtung_tot where quelle='AFO' and dubletten_nr is not null;
select count(*) from google_maps_dev_abgleich.poi_abgleich_google_religioese_einrichtung_tot where quelle='GOOGLE' and dubletten_nr is not null;
*/
	...
	aws_s3.query_export_to_s3(
		'	
			select 
				* 
			from 
				google_maps_dev_abgleich.poi_abgleich_google_religioese_einrichtung_tot
		', 
   		aws_commons.create_s3_uri(
   			'afo-db-prod-data-exchange', 
   			'google_maps_dev_abgleich/poi_abgleich_google_religioese_einrichtung_tot.csv', 
   			'eu-central-1'
   		),
