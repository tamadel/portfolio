...
------------------------------------------
-- Dienstleistung all categories
------------------------------------------
/*
ALTER TABLE 
	google_maps_dev.google_map_metadata_dienstleistung
...
select 
	*
from 
	google_maps_dev.google_map_results_dienstleistung
;
...
select 
	*
from 
	google_maps_dev.google_map_items_dienstleistung
;
...
-- pois number: 18650
create temp table tmp_dienstleistung_category_v1
as
select 
	*
from 
	google_maps_dev.google_map_items_dienstleistung t0
where 
	category_ids = 'null'
	or
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t2.hauptkategorie_neu = 'Dienstleistung'
	)  
;
...
select 
	...
	,count(*)
from
	tmp_dienstleistung_category_v1
group by 
	cid
having 
	count(*) > 1
...
-- pois number: 17303
create temp table tmp_dienstleistung_category_v2
as
select 
	*
from 
...
where 
	...
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t1.hauptkategorie_neu = 'Dienstleistung'
	)
;
...
where 
	cid not in (
				select 
					cid
				from
					tmp_dienstleistung_category_v2
	)
;
...
drop table if exists tmp_dienstleistung_category;
create temp table 
		tmp_dienstleistung_category
as
select 
	cid
	,exact_match
...
--===============================================
drop table if exists tmp_google_map_dienstleistung;
create temp table tmp_google_map_dienstleistung
as
select
	t0.cid
	,t0.exact_match
	...
	,t0.latitude
from 
	tmp_dienstleistung_category t0
left join
...
select * from tmp_google_map_dienstleistung;
...
-- (2.1) OPENING HOURS
alter table 
	tmp_google_map_dienstleistung
add column if not exists 
	opening_times text
;

--alter table tmp_google_map_dienstleistung
--drop column opening_times
--;

update tmp_google_map_dienstleistung
set  
    opening_times = case 
...
update
	tmp_google_map_dienstleistung t0
set
	plz6 = t1.plz6
from
...
update tmp_google_map_dienstleistung t0
set 
	plz_ort = t1.ort
from 
...
--=================================
create index idx_tmp_googl_missing_hostel_poly_lv95 on tmp_google_map_dienstleistung using GIST(geo_point_lv95);
...
drop table if exists tmp_google_map_dienstleistung_V1;
...
create temp table tmp_google_map_dienstleistung_V1
as
select 
    cid
    ,bezeichnung
...
select * from tmp_google_map_dienstleistung_V1;
--============================================================================================
...
alter table tmp_google_map_dienstleistung_V1
...
select
	...
	,gwr_strasse 
from
	tmp_google_map_dienstleistung_V1
where 
	--google_plz4 = plz4
	--google_plz4 <> plz4
	google_plz4 is null
...
--Korr_plz4
update tmp_google_map_dienstleistung_V1
set
	korr_plz4 = plz4
where	
	korr_plz4 IS NULL
...
update tmp_google_map_dienstleistung_V1 t0
set 
...
--STEP(9) Final Table
--=====================================
drop table if exists google_maps_dev.google_map_dienstleistung;
...
drop table if exists google_maps_dev_abgleich.google_abgleich_dienstleistung;
create table google_maps_dev_abgleich.google_abgleich_dienstleistung
as
select
	 cid as cid 
...
--POIS: AFO dienstleistungdienst
create table google_maps_dev_abgleich.afo_poi_typ_dienstleistung
as
select 
	*
from 
