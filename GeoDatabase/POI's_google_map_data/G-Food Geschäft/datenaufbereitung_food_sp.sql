...
-- pois number: 18650
create temp table tmp_food_category_v1
as
select 
	*
from 
	google_maps_dev.google_map_items_food_geschäft_all t0
where 
	category_ids = 'null'
	or
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t2.hauptkategorie_neu = 'Food-Geschäft'
	)  
;
...
-- pois number: 17303
create temp table tmp_food_category_v2
as
select 
	*
from 
...
where 
	...
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t1.hauptkategorie_neu = 'Food-Geschäft'
	)
;
...
-- SPSPSP Schauen, ob es viele gibt ohne irgendwelche Categories -> Ja, anscheinend schon

SELECT 
	* 
FROM 
	google_maps_dev.google_map_results_food_geschäft_all 
WHERE 
	cid = '1109884709447366513';
...
drop table if exists tmp_food_category;
create temp table 
		tmp_food_category
as
select 
...
--===============================================
drop table if exists tmp_google_map_food;
create temp table tmp_google_map_food
as
select
	t0.cid
	,t0.exact_match
...
-- select * from tmp_google_map_food;


--==================================
-- STEP(2) Working_hours (Reformat)
--==================================
/* -- in case (B) Python script: google_maps_get_new.py
create temp table tmp_google_map_<Hauptkategorie-Name>
as 
select
	*
from 
	google_map_<Hauptkategorie-Name>
;
*/
-- (2.1) OPENING HOURS
alter table 
...
--=================================
create index idx_tmp_googl_missing_hostel_poly_lv95 on tmp_google_map_food using GIST(geo_point_lv95);
...
drop table if exists tmp_google_map_food_V1;
...
create temp table tmp_google_map_food_V1
as
select 
    cid
    ,bezeichnung
...
-- select * from tmp_google_map_food_V1;
--============================================================================================
...
alter table tmp_google_map_food_V1
...
update tmp_google_map_food_V1 t0
set 
...
--STEP(9) Final Table
--=====================================
drop table if exists google_maps_dev.google_map_food_v1;
...
select 
	...
	,count(*)
from 
	google_maps_dev.google_map_dienstleistung
group by
	cid
having 
	count(*) > 1
...
update google_maps_dev.google_map_dienstleistung
set 
	korr_plz4 = '8004'
where 
...
drop table if exists google_maps_dev_abgleich.google_abgleich_food;

create table google_maps_dev_abgleich.google_abgleich_food
as
select
	 cid as cid 
...
from 
	google_maps_dev.google_map_food_v1
WHERE 
	LENGTH(TRIM(korr_plz4)) = 4 -- Zum Rausfiltern der falschen PLZ4 - SPSPSP
;

-- SPSPSP einige korrigierte PLZ4 sind nur falsch
SELECT 
	*
from 
	google_maps_dev.google_map_food_v1
WHERE 
	LENGTH(TRIM(korr_plz4)) <> 4
;

--POIS: Google Food -- to Redshift
select 
	*
from 
...
--POIS: AFO FOOD -- to Redshift

DROP TABLE IF EXISTS 
	google_maps_dev.afo_poi_typ_food;
...
DROP TABLE IF EXISTS 
	google_maps_dev.export_afo_poi_typ_food;
...
ALTER TABLE 
	google_maps_dev.export_afo_poi_typ_food
ADD COLUMN IF NOT EXISTS 
	geo_point_lv03_text VARCHAR(1000),
ADD COLUMN IF NOT EXISTS 
	geo_point_lv95_text VARCHAR(1000),
ADD COLUMN IF NOT EXISTS 
	geo_point_wgs84_text VARCHAR(1000);

UPDATE 
	google_maps_dev.export_afo_poi_typ_food
SET 
	geo_point_lv03_text = ST_AsText(geo_point_lv03),
	geo_point_lv95_text = ST_AsText(geo_point_lv95),
	geo_point_wgs84_text = ST_AsText(geo_point_wgs84);
...
-- Welche gehören zu Food? Oben einsetzen.
