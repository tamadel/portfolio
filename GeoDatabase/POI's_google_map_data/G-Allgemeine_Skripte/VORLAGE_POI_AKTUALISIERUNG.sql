...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_<HAUPTKATEGORIE>_v1 a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 12 and poi_typ_id_list is null;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 2 = any(poi_main_kat_id_list);
