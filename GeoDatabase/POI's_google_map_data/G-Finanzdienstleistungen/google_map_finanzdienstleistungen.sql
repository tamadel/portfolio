--===========================================
--Hauptkategorie = 'Finanzdienstleistungen'
--
--===========================================
...
where 
	hauptkategorie_neu = 'Finanzdienstleistungen'
;
...
------------------------------------------
-- Finanzdienstleistungen all categories
------------------------------------------
ALTER TABLE 
	google_maps_dev.google_map_metadata_finanzdienstleistungen
...
select 
	...
	,title
from
	google_maps_dev.google_map_results_finanzdienstleistungen
where 
	lower(title) like '%ubs bancomat%'
;
...
select 
	*
from
	google_maps_dev.google_map_items_finanzdienstleistungen
where 
	lower(title) like '%ubs%'
;
...
-- pois number: 18650
create temp table tmp_finanz_category_v1
as
select 
	*
from 
	google_maps_dev.google_map_items_finanzdienstleistungen t0
where 
	category_ids = 'null'
	or
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t2.hauptkategorie_neu = 'Finanzdienstleistungen'
	)  
;


-- VERSION(2): Filter with considering only the first index of category_ids
-- pois number: 17303
create temp table tmp_finanz_category_v2
as
select 
	*
from 
...
where 
	...
	exists (
		    ...
		    where
		    	...
		    	and 
		    	t1.hauptkategorie_neu = 'Finanzdienstleistungen'
	)
;

-- Check the difference to decide which version I am going to use
-- pois number: 1347
select 
	*
from 
	tmp_finanz_category_v1
where 
	cid not in (
				select 
					cid
				from
					tmp_finanz_category_v2
	)
;


--PS.: I will continue with VERSION(1) 
--==============================================================
...
drop table if exists tmp_finanz_category;
create temp table 
		tmp_finanz_category
as
select 
...
--===============================================
drop table if exists tmp_google_map_finanzdienstleistung;
create temp table tmp_google_map_finanzdienstleistung
as
select
	t0.cid
	,t0.exact_match
...
select * from tmp_google_map_finanzdienstleistung;
...
-- (2.1) OPENING HOURS
alter table 
	tmp_google_map_finanzdienstleistung
add column if not exists 
	opening_times text
;

--alter table tmp_google_map_finanzdienstleistung
--drop column opening_times
--;

update tmp_google_map_finanzdienstleistung
set  
    opening_times = case 
...
update
	tmp_google_map_finanzdienstleistung t0
set
	plz6 = t1.plz6
from
...
update tmp_google_map_finanzdienstleistung t0
set 
	plz_ort = t1.ort
from 
...
--=================================
create index idx_tmp_googl_missing_hostel_poly_lv95 on tmp_google_map_finanzdienstleistung using GIST(geo_point_lv95);
...
drop table if exists tmp_google_map_finanzdienstleistung_V1;
...
create temp table tmp_google_map_finanzdienstleistung_V1
as
select 
    cid
    ,bezeichnung
...
select * from tmp_google_map_finanzdienstleistung_V1;
--============================================================================================
...
alter table tmp_google_map_finanzdienstleistung_V1
...
select
	...
	,gwr_strasse 
from
	tmp_google_map_finanzdienstleistung_V1
where 
	--google_plz4 = plz4
	--google_plz4 <> plz4
	google_plz4 is null
...
--Korr_plz4
update tmp_google_map_finanzdienstleistung_V1
set 
	korr_plz4 = plz4
where  
	google_plz4 = plz4
...
update tmp_google_map_finanzdienstleistung_V1 t0
set 
...
--STEP(9) Final Table
--=====================================
drop table if exists google_maps_dev.google_map_finanzdienstleistung;

create table 
	google_maps_dev.google_map_finanzdienstleistung
as
select 
	cid    
	,bezeichnung 
...
drop table if exists google_maps_dev_abgleich.google_bbgleich_finanzdienstleistung;
create table google_maps_dev_abgleich.google_abgleich_finanzdienstleistung
as
select
	 cid
...
--POIS: Google Finanzdienst
select 
	*
from 
	google_maps_dev_abgleich.google_abgleich_finanzdienstleistung
where 
	lower(title) like '%bank%'
	or 
	lower(title) like '%ubs atm%'
;
...
select 
	*
from 
	google_maps_dev.google_map_items_finanzdienstleistung
...
--POIS: AFO Finanzdienst
create table google_maps_dev_abgleich.afo_poi_typ_finanzdienstleistung
as
select 
	*
from 
