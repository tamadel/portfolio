...
-- select * from google_maps_dev_abgleich.poi_abgleich_google_food_tot_clean where poi_typ_id <> 102 and poi_typ_id <> 101 order by dubletten_nr, quelle;
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_food_geschäft_all t0
			left join
				tmp_poi_gm_match t1
			on
...
from (
	select 
		...
		,a.company
	from
		google_maps_dev_abgleich.poi_abgleich_google_food_tot_clean a
	join 
		tmp_poi_gm_match b
	on
		a.poi_id = b.cid
...
where
	...
	poi_typ_id_old in (
		...
		where 
			poi_main_kat_neu = 'Food-Geschäft'
	)
	and 
	poi_typ_id_old <> 207		-- ohne Food
	or (
		poi_typ_id_old = 207
		and
		company_id not in (
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 2 and poi_typ_id_list is null;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 7 = any(poi_main_kat_id_list);
