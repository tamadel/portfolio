...
where
	...
	and
	poi_typ_id <> 102		-- ohne Banken
	and
	poi_typ_id <> 101		-- ohne Bancomat
;
...
from
	google_maps_dev_abgleich.poi_abgleich_google_finanzdienstleistung_tot u
where
	u.quelle = 'AFO'
	and
	r.dubletten_nr = u.dubletten_nr 
	and
	u.poi_typ_id <> 102		-- ohne Banken
	and
	poi_typ_id <> 101		-- ohne Bancomat
;
delete from 
...
-- select * from google_maps_dev_abgleich.poi_abgleich_google_finanzdienstleistung_tot where poi_typ_id <> 102 and poi_typ_id <> 101 order by dubletten_nr, quelle;
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_finanzdienstleistung t0
			left join
				tmp_poi_gm_match t1
			on
...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_finanzdienstleistung a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
from (
	...
	from
		tmp_poi_gm_match t0
	join
		google_maps_dev.google_map_finanzdienstleistung t1
	on
		t0.cid::text = t1.cid::text
) u
where
...
from (
	...
	where
		extract(year from gueltig_bis) = 9999
		and
		poi_main_kat = 'Finanzdienstleistungen'
) u
where
	extract(year from r.gueltig_bis) = 9999
	and
...
from (
	select 
		...
		,a.company
	from
		google_maps_dev_abgleich.poi_abgleich_google_finanzdienstleistung_tot a
	join 
		tmp_poi_gm_match b
	on
		a.poi_id = b.cid
...
where
	...
	poi_typ_id_old in (
		...
		where 
			poi_main_kat_neu = 'Finanzdienstleistungen'
	)
	and 
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 1 and poi_typ_id_list is null;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 6 = any(poi_main_kat_id_list);
...
from
	google_maps_dev.google_map_finanzdienstleistung_v1 a
left join
	tmp_poi_gm_match b
on
