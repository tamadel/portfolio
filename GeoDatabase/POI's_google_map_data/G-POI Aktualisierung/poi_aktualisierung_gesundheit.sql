...
from
	google_maps_dev_abgleich.poi_abgleich_google_gesundheit_tot u
where
	u.quelle = 'AFO'
	and
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_gesundheit_v1 t0
			left join
				tmp_poi_gm_match t1
			on
...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_gesundheit_v1 a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
-------------------------------------------------------------------------------------
-- select * from tmp_poi_gm_match where cid in (select cid from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999);
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and cid = '6832889554936679778';
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_id = 6408;
-- select * from google_maps_dev.google_map_gesundheit_v1 where cid = '6209378964125343312';
update
	geo_afo_prod.lay_poi_geo_2024_hist r
set
	cid = u.cid
...
from (
	...
	from
		tmp_poi_gm_match t0
	join
		google_maps_dev.google_map_gesundheit_v1 t1
	on
		t0.cid::text = t1.cid::text
) u
where
...
from (
	select 
		...
		,a.company
	from
		google_maps_dev_abgleich.poi_abgleich_google_gesundheit_tot a
	join 
		tmp_poi_gm_match b
	on
		a.poi_id = b.cid
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select distinct poi_typ_id_old from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 4 and poi_main_kat_id_list is null;
-- select * from google_maps_dev_abgleich.map_poi_typ_alt_neu where poi_typ_id_alt = 307;
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select distinct poi_typ_id_old from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 4 and poi_kat_id_list is null;
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 4 and poi_typ_id_list is null;
-- select count(*) from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 9 = any(poi_main_kat_id_list);
-- select count(*) from tmp_poi_current;
