...
from
	google_maps_dev_abgleich.poi_abgleich_google_mobilitaet_tot u
where
	u.quelle = 'AFO'
	and
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_mobilitaet_v1 t0
			left join
				tmp_poi_gm_match t1
			on
...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_mobilitaet_v1 a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
from (
	...
	from
		tmp_poi_gm_match t0
	join
		google_maps_dev.google_map_mobilitaet_v1 t1
	on
		t0.cid::text = t1.cid::text
) u
where
...
from (
	select 
		...
		,a.company
	from
		google_maps_dev_abgleich.poi_abgleich_google_mobilitaet_tot a
	join 
		tmp_poi_gm_match b
	on
		a.poi_id = b.cid
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select distinct poi_typ_id_old from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 13 and poi_main_kat_id_list is null;
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select distinct poi_typ_id_old from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 13 and poi_kat_id_list is null;
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date order by updated_ts desc;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and poi_kat_id_old = 13 and poi_typ_id_list is null;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 12 = any(poi_main_kat_id_list);
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and 12 = any(poi_main_kat_id_list) and trim(bezeichnung_lang) = '';
