...
from
	google_maps_dev_abgleich.poi_abgleich_google_hotel_gastro_tot u
where
	u.quelle = 'AFO'
	and
...
-- select * from google_maps_dev_abgleich.poi_abgleich_google_hotel_gastro_tot where quelle = 'GOOGLE' and dubletten_nr is not null;
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_hotel_gastronomie_v5 t0
			left join
				tmp_poi_gm_match t1
			on
...
where
	b.hauptkategorie_neu = 'Hotel & Gastronomie'
;
...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_hotel_gastronomie_v5 a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
where
	...
	r.poi_id = u.poi_id
;
-- select count(*) from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date;
...
from (
	...
	from
		tmp_poi_gm_match t0
	join
		google_maps_dev.google_map_hotel_gastronomie_v5 t1
	on
		t0.cid::text = t1.cid::text
) u
where
	...
	r.cid is null
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where updated_ts::date = current_date;
