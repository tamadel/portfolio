...
--===============================================================================
select * from google_maps_dev_abgleich.poi_abgleich_google_bildung_tot where quelle = 'GOOGLE' and dubletten_nr is null;
-------------------------------------------------------------------------------------
...
from
	google_maps_dev_abgleich.poi_abgleich_google_bildung_tot u
where
	u.quelle = 'AFO'
	and
...
-- select quelle, count(*) from google_maps_dev_abgleich.poi_abgleich_google_bildung_tot where dubletten_nr is not null group by quelle;
...
with
	category_id as (
		...
		from (
			...
			from
				google_maps_dev.google_map_bildung_v1 t0
			left join
				tmp_poi_gm_match t1
			on
...
-- select * from google_maps_dev.google_map_hotel_gastronomie_v5;
...
from (
	select
		...
		,a.geo_point_google
	from
		google_maps_dev.google_map_bildung_v1 a
	join
		tmp_poi_gm_match b
	on
		a.cid = b.cid
...
from (
	...
	from
		tmp_poi_gm_match t0
	join
		google_maps_dev.google_map_bildung_v1 t1
	on
		t0.cid::text = t1.cid::text
) u
where
	...
	r.cid is null
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and updated_ts::date = current_date;

-------------------------------------------------------------------------------------
-- bei gematchten POIs poi_main_kat_id in lay_poi_geo_hist setzen, falls leer
-------------------------------------------------------------------------------------
update
	geo_afo_prod.lay_poi_geo_2024_hist r
set
	poi_main_kat_id = ARRAY_APPEND(r.poi_main_kat_id, u.poi_main_kat_id)
	,updated_ts = current_timestamp
from 
	tmp_poi_map u
where
...
;
-- select * from geo_afo_prod.lay_poi_geo_2024_hist where extract(year from gueltig_bis) = 9999 and cid is not null;
...
create table geo_afo_tmp.lay_poi_geo_hist_test_ta
as
select
	*
from 	
