...
SELECT *
FROM #base_koord
WHERE 000(ST_SetSRID(geo_point_lv95::geometry, 2056), ST_SetSRID(geo_point_lv95_b::geometry, 2056), 30); --ST_DWithin(geo_point_lv95::geometry, geo_point_lv95_b::geometry, 30);
...
select 
	...
	cast('' as varchar(255)) as category_ids,
	t1.company_group_id,
	t1.company_group,
...
where dubletten_nr is not null;
  
select dubletten_nr, count(*)
from geo_afo_prod.poi_abgleich_google_tot
where dubletten_nr is not null
group by dubletten_nr
having
count(*) > 1
order by count(*) desc; 
...
SELECT address,strasse, gwr_strasse,public.search_leerschlag(address,2,',')
FROM geo_afo_prod.google_hotel_gastro
where REGEXP_COUNT(address,',',1)=3
LIMIT 100;

--Intersection With, via della Posta 24, Via Francesco Ballerini 14, 6600 Locarno
--Sankt Johanns-Rheinweg 109 c/o bisskid AG, Baselstrasse 71, 4142 Münchenstein, 4056 Basel



drop table if exists tmp_poi_abgleich_google_tot_csv;
create temp table tmp_poi_abgleich_google_tot_csv
as
select
	*
from 
	geo_afo_prod.poi_abgleich_google_tot
	--geo_afo_prod.poi_abgleich_google_finanzdienstleistung_tot
;	


update tmp_poi_abgleich_google_tot_csv
set poi_id = '''' || poi_id,
...
select 
	*
from 
	tmp_poi_abgleich_google_tot_csv
where 
	company like '%Weissfluehjoch%'
order by
	dubletten_nr
;
...
select
	*
from
	geo_afo_prod.google_hotel_gastro 
where 
	cid in ('1584772495961605602','16140773433072100578','16349566801690119706')
;
