...
/*
SELECT *
FROM geo_afo_prod.google_afo_mobilität
LIMIT 10;
...
SELECT *
FROM geo_afo_prod.google_map_abgleich_mobilität
LIMIT 10;
...
from geo_afo_prod.google_afo_mobilität;  -- Namen anpassen Poi Tabelle
...
from geo_afo_prod.google_map_abgleich_mobilität; -- Namen anpassen Google Tabelle
...
from #strasse 
where plz4 = 0 and substring(address,1,1) in ('1','2','3','4','5','6','7','8','9') and length(trim(substring(address, 1, regexp_instr(address, ' [^ ]*'))))=4                  
;
...
FROM geo_afo_prod.poi_mch AS a LEFT JOIN geo_afo_prod.google_mch AS b
	ON (a.strasse_std=b.strasse_std AND a.hnr_std=b.hnr_std AND a.plz_grob=b.plz_grob and len(a.plz_grob)>2)
...
FROM geo_afo_prod.poi_mch AS a LEFT JOIN geo_afo_prod.google_mch AS b
	ON (a.strasse_std=b.strasse_std AND a.plz_grob=b.plz_grob and len(a.plz_grob)>2) and (a.hnr_std='' or b.hnr_std='')
...
FROM geo_afo_prod.poi_mch AS a LEFT JOIN geo_afo_prod.google_mch AS b
	ON (a.strasse_std1=b.strasse_std1 AND a.hnr_std=b.hnr_std AND a.plz_grob=b.plz_grob and len(a.plz_grob)>2)
		and public.levenshtein_levenshtein_token_set_ratio(a.firma_std_ohne_phon, b.firma_std_ohne_phon)>90 and a.firma_std_ohne_phon<>''
WHERE b.cid IS NOT null;
...
SELECT *
FROM #base_koord
WHERE ST_DWithin(geo_point_lv95::geometry, geo_point_lv95_b::geometry, 30);
...
--------------------------------------------------------------------
-- 4. Zusammenfassung Dubletten für Peter - Spalten ergänzen
---------------------------------------------------------------------

-- Dubletten File mit AZ Adressen und TopCC Adressen
drop table if exists geo_afo_prod.poi_abgleich_google_tot_mobilität;
create table geo_afo_prod.poi_abgleich_google_tot_mobilität as
select 
...
insert into geo_afo_prod.poi_abgleich_google_tot_mobilität
select 
	T1.cid, 
	0 as hauskey,
	0 as poi_typ_id,
	...
	'GOOGLE' as quelle,
	cast(t2.cid as varchar(255)) as dubletten_nr 
from geo_afo_prod.google_map_abgleich_mobilität T1 -- Namen anpassen
...
select *
from geo_afo_prod.poi_abgleich_google_tot_mobilität
order by random() limit 20

select *
from geo_afo_prod.poi_abgleich_google_tot_mobilität
where plz4=8404 order by dubletten_nr, company
